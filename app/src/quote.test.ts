...
describe("estimateTotalCents", () => {
  ...
  it("відхиляє небезпечну ставку навіть при нульових годинах", () => {
    // Дзеркало попереднього випадку: 2 ** 54 формально ціле (isInteger → true),
    // але вже не відрізняється від сусідів. З hours: 0 добуток виходив 0 —
    // безпечний — і ставка не перевірялась узагалі. Добуток нічого не доводить
    // про свої множники.
    expect(() => estimateTotalCents({ hours: 0, rateCents: 2 ** 54 })).toThrow(RangeError);
  });

  it("відхиляє небезпечні масштабовані години навіть при нульовій ставці", () => {
    // 1e14 * 100 = 1e16 — за межею точного цілого. Добуток із нульовою ставкою
    // давав 0, тож перевірка grossCenti мовчала, а самі centiHours ніхто не
    // перевіряв. Тепер операнд перевіряється там, де народився.
    expect(() => estimateTotalCents({ hours: 1e14, rateCents: 0 })).toThrow(RangeError);
  });

  it("відхиляє точніші за контракт години й на великих величинах", () => {
    // Суто відносний допуск ріс разом із product і при |product| >= 0.5/EPSILON
    // дотягувався до половини одиниці сітки: 22517998136852.484 * 100 давало
    // дріб рівно 0.5 і проходило перевірку точності. Стеля допуску це закриває.
    expect(() =>
      estimateTotalCents({ hours: 22517998136852.484, rateCents: 0 }),
    ).toThrow(RangeError);
  });
