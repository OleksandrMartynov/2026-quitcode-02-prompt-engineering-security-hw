...
function toScaled(name: string, value: number, scale: number): number {
  ...
  // Допуск відносний **і обмежений зверху**. Обидві половини потрібні.
  // Відносність: абсолютний `1e-9` пропускав `hours: 1e-12` (`product = 1e-10`)
  // — округлювалось до 0 і тихо проходило попри контракт.
  // Обмеження: сама лише відносність росте разом із `product` і при
  // `|product| >= 0.5 / EPSILON` (≈ 2.25e15) дотягується до половини одиниці
  // сітки — тобто починає ковтати справжній дріб. `hours: 22517998136852.484`
  // (три знаки після коми) так і проходило перевірку точності.
  // Стеля `1/4` одиниці лишається вищою за будь-яку похибку представлення
  // двознакового значення, але нижчою за найменший дріб, який сітка здатна
  // відрізнити. Там, де відрізнити вже неможливо, вхід відхиляється — гучна
  // помилка краща за тихо округлений кошторис.
  const tolerance = Math.min(0.25, Number.EPSILON * Math.max(1, Math.abs(product)));
